...
import {
  ...
  projectIconMimeType,
  projectIconScore,
} from "./project-artwork.server";

describe("project artwork policy", () => {
  it.each([
    ...
    const artwork = await findProjectArtwork(
      {
        ...
        readFile: async () => {
          const content = JSON.stringify({ bb: { branding: { icon: "GridView" } } });
          return { content, contentEncoding: "utf8", mimeType: "application/json", sizeBytes: content.length };
        },
      },
      "project-1",
      new AbortController().signal,
    ...
    const artwork = await findProjectArtwork(
      {
        ...
        readFile: async ({ path }) => {
          if (path === "package.json") {
            const content = JSON.stringify({ bb: { branding: { icon: "NotABbIcon" } } });
            return { content, contentEncoding: "utf8", mimeType: "application/json", sizeBytes: content.length };
          }
          return { content: "iVBORw0KGgo=", contentEncoding: "base64", mimeType: "image/png", sizeBytes: 8 };
        },
      },
      "project-1",
      new AbortController().signal,
  ...
  it("skips an unsafe SVG and serves the next safe bitmap", async () => {
    const icon = await findProjectArtwork(
      {
        listFiles: async ({ query }) => ({
          files: query === "icon" ? [{ path: "icon.svg" }, { path: "logo.png" }] : [],
        }),
        readFile: async ({ path }) => {
          if (path === "package.json") throw new Error("not found");
